    ...
    <section className="mwh" style={{ transform: `scale(${scale})` }} data-screen-label="HERO"
      ...
      <header className="mwh-header">
        ...
        <div className="mwh-search">
          ...
          <button className="go" aria-label="search" onClick={() => nav('SCR-002')}>
        ...
        <div className="right">
          ...
          <button className="mwh-burger" aria-label="menu" onClick={() => nav('SCR-002')}><span></span><span></span><span></span></button>
        </div>
      </header>
  ...
  return (
    <footer className="site-footer">
        ...
        <div>
          ...
          <ul><li style={{ cursor: 'pointer' }} onClick={() => nav('SCR-006')}>会員登録</li><li style={{ cursor: 'pointer' }} onClick={() => nav('SCR-007')}>ログイン</li><li style={{ cursor: 'pointer' }} onClick={() => nav('SCR-008')}>マイページ</li></ul>
        </div>
        <div>
          <div className="ft-title">FOR COMPANY</div>
          <ul><li style={{ cursor: 'pointer' }} onClick={() => nav('SCR-010')}>掲載プラン</li><li style={{ cursor: 'pointer' }} onClick={() => nav('SCR-015')}>お問い合わせ</li><li style={{ cursor: 'pointer' }} onClick={() => nav('SCR-016')}>企業ログイン</li></ul>
        </div>
        <div>
          <div className="ft-title">ABOUT</div>
          <ul><li style={{ cursor: 'pointer' }} onClick={() => nav('SCR-010')}>Discoveryとは</li><li style={{ cursor: 'pointer' }} onClick={() => nav('SCR-015')}>利用規約</li><li style={{ cursor: 'pointer' }} onClick={() => nav('SCR-015')}>プライバシー</li><li style={{ cursor: 'pointer' }} onClick={() => nav('SCR-015')}>FAQ</li></ul>
        </div>
      </footer>);
  ...
  return (
    <div className="job-card" onClick={() => nav('SCR-003', { job })}>
      <div className="job-card-thumb" onClick={(e) => e.stopPropagation()}>
        <image-slot id={"thumb-" + job.id} src={job.img} shape="rect" placeholder="求人画像" style={{ display: 'block', width: '100%', height: '100%' }}></image-slot>
      ...
      <div className="bottom">
        ...
        <button className="icon-btn" style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--color-text-primary)', color: '#fff' }}>→</button>
      </div>
    </div>);
...
// ============ SEARCH PAGE (求人一覧) ============
const JL_PIN = <i className="jl-pin" aria-hidden="true"></i>;

// 求人一覧の表示データ（参照デザインに準拠）。詳細画面用のフィールドも保持。
const JOBLIST = [
...
function JobTags({ tags }) {
  ...
  React.useLayoutEffect(() => {
    ...
    const compute = () => {
      ...
      const tagEls = Array.from(meas.querySelectorAll('[data-tag]'));
      const plusEl = meas.querySelector('[data-plus]');
      ...
      for (let k = 1; k <= tags.length; k++) {
        ...
        if (sum + need <= containerW) best = k; else break;
      }
  ...
  return (
    <div className="jl-tags" ref={wrapRef}>
      <div className="jl-tags-measure" ref={measureRef} aria-hidden="true">
        {tags.map((t) => <span key={t} className="jl-tag" data-tag>{t}</span>)}
        <span className="jl-tag jl-tag-more" data-plus>+{tags.length}</span>
      </div>
      {tags.slice(0, shown).map((t) => <span key={t} className="jl-tag">{t}</span>)}
      {overflow > 0 && <span className="jl-tag jl-tag-more">+{overflow}</span>}
    </div>);

}

function JobListCard({ job, nav }) {
  const [fav, setFav] = useState(job.fav);
  return (
    <article className="jl-card" onClick={() => nav('SCR-003', { job })}>
      <div className="jl-thumb">
        <image-slot id={"jlthumb-" + job.id} src={job.img} shape="rect" placeholder="求人画像" style={{ display: 'block', width: '100%', height: '100%' }}></image-slot>
        <button className={"jl-bookmark" + (fav ? ' on' : '')} aria-label="保存" onClick={(e) => {e.stopPropagation();setFav(!fav);}}>
          <svg width="15" height="19" viewBox="0 0 24 30" fill={fav ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2"><path d="M3 2h18v26l-9-7-9 7z" /></svg>
        </button>
      </div>
      <div className="jl-body">
        ...
        <div className="jl-meta"><span>{JL_PIN}{job.loc}</span></div>
...
function FilterRow({ label, value, options }) {
  ...
  return (
    <div className={"jl-filter" + (open ? ' open' : '')}>
      <button className="jl-filter-head" onClick={() => setOpen(!open)}>
      ...
      {open &&
      <div className="jl-filter-opts">
          {options.map((o) =>
        <label key={o} className="jl-filter-opt"><input type="checkbox" />{o}</label>
        )}
        </div>
      }
    </div>);
    ...
    <div className="jl">
      ...
      <header className="jl-hero">
        <div className="jl-hero-left">
          ...
          <p className="jl-hero-lead">音楽・エンタメ業界の最新求人情報を掲載中。気になる求人を見つけて応募しよう。</p>
        </div>
        <div className="jl-control">
          <div className="jl-count"><b>152</b>件<span className="jl-count-sort">（新着順）</span></div>
      ...
      <div className="jl-layout">
        <div className="jl-main">
          <div className="jl-grid">
            {JOBLIST.map((j) => <JobListCard key={j.id} job={j} nav={nav} />)}
          </div>

          <div className="pagination jl-pagination">
            <button aria-label="前へ" onClick={() => nav('SCR-002')}>‹</button>
            <button className="on" onClick={() => nav('SCR-002')}>1</button>
            <button onClick={() => nav('SCR-002')}>2</button>
            <button onClick={() => nav('SCR-002')}>3</button>
            <button className="dots" disabled>…</button>
            <button onClick={() => nav('SCR-002')}>10</button>
            <button aria-label="次へ" onClick={() => nav('SCR-002')}>›</button>
          </div>
        </div>
        ...
        <aside className="jl-side">
          <div className="jl-filter-section">
            ...
            <div className="jl-field">
              <label className="jl-field-label">キーワード</label>
              <div className="jl-search">
                <input placeholder="職種名・企業名・スキルなど" />
                <button aria-label="検索">
                  <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7"><circle cx="11" cy="11" r="7" /><line x1="21" y1="21" x2="16.5" y2="16.5" /></svg>
                </button>
              </div>
            </div>

            <FilterRow label="職種" value="すべての職種" options={data.categories.map((c) => c.ja)} />
            <FilterRow label="雇用形態" value="すべての雇用形態" options={['正社員', '契約社員', '業務委託', 'アルバイト・パート']} />
            <FilterRow label="勤務地" value="すべての勤務地" options={['東京', '大阪', '名古屋', '福岡', 'リモート可']} />
            <FilterRow label="タグ" value="すべてのタグ" options={['フェス', 'PA/音響', 'ライブ', '編集', 'DTM', 'プロモーション']} />

            <div className="jl-actions">
              <button className="jl-search-btn" onClick={() => nav('SCR-002')}>この条件で検索する</button>
              <button className="jl-reset-btn" onClick={() => nav('SCR-002')}>条件をリセット</button>
            </div>
          </div>
    ...
    <div className="detail">
      <div>
        ...
        <div className="job-detail-visual" onClick={(e) => e.stopPropagation()}>
          <image-slot id={"thumb-" + j.id} src={j.img} shape="rect" placeholder="求人のキービジュアル（後で差し替え）" style={{ display: 'block', width: '100%', height: '100%' }}></image-slot>
        </div>
  ...
  return (
    <div className="mypage">
      <div className="mypage-header">
        ...
        <button className="btn-ghost" onClick={() => setTab('profile')}>プロフィール編集</button>
      </div>
      ...
      <div className="form-card" style={{ maxWidth: 720 }}>
          ...
          <button className="btn-primary" onClick={() => setTab('history')}>変更を保存 ↗</button>
        </div>
      }
...
// Export
Object.assign(window, { TopPage, SearchPage, JobDetail, ApplyPage, ApplyDone, MyPage, JobCard, SiteFooter });